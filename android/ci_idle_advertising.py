...
code=r'''    protected boolean idleAdsEnabled=false;protected int idleAdsSeconds=120,idleAdsRotateSeconds=15,idleAdIndex=0;protected JSONArray idleAds=new JSONArray();private android.widget.ImageView idleAdView;private android.widget.FrameLayout idleAdOverlay;private final Runnable idleAdStart=()->showIdleAdvertising();private final Runnable idleAdRotate=()->rotateIdleAdvertising();
    protected void loadIdleAdvertising(){io.execute(()->{try{JSONArray cfg=requestArray("GET","/rest/v1/idle_ad_settings?id=eq.1&select=*",null,true);JSONArray ads=requestArray("GET","/rest/v1/idle_ads?is_active=eq.true&select=*&order=sort_order.asc,created_at.asc",null,true);if(cfg.length()>0){JSONObject c=cfg.optJSONObject(0);idleAdsEnabled=c.optBoolean("enabled",false);idleAdsSeconds=Math.max(30,c.optInt("idle_seconds",120));idleAdsRotateSeconds=Math.max(5,c.optInt("rotate_seconds",15));}idleAds=ads;main.post(this::resetIdleAdvertisingTimer);}catch(Exception ignored){}});}
    protected void resetIdleAdvertisingTimer(){main.removeCallbacks(idleAdStart);hideIdleAdvertising();getWindow().addFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);if(idleAdsEnabled&&idleAds.length()>0)main.postDelayed(idleAdStart,idleAdsSeconds*1000L);}
    private void showIdleAdvertising(){if(!idleAdsEnabled||idleAds.length()==0)return;getWindow().addFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);if(idleAdOverlay==null){idleAdOverlay=new android.widget.FrameLayout(this);idleAdOverlay.setBackgroundColor(Color.BLACK);idleAdView=new android.widget.ImageView(this);idleAdView.setScaleType(android.widget.ImageView.ScaleType.FIT_CENTER);idleAdOverlay.addView(idleAdView,new android.widget.FrameLayout.LayoutParams(-1,-1));idleAdOverlay.setClickable(true);idleAdOverlay.setFocusable(true);idleAdOverlay.setOnTouchListener((v,e)->{if(e.getAction()==android.view.MotionEvent.ACTION_DOWN){resetIdleAdvertisingTimer();return true;}return true;});addContentView(idleAdOverlay,new android.widget.FrameLayout.LayoutParams(-1,-1));}idleAdOverlay.bringToFront();idleAdOverlay.setVisibility(View.VISIBLE);idleAdIndex=0;rotateIdleAdvertising();}
...
'''
if 'protected void loadIdleAdvertising()' in s or 'private void loadIdleAdvertising()' in s:
    a=s.find('    private boolean idleAdsEnabled=')
    if a<0:a=s.find('    protected boolean idleAdsEnabled=')
    b=s.find(marker,a)
...
print('Idle advertising kept active in kiosk mode')
