...
# Remaining references inside the inventory-history UI lambda must also use the final alias.
start=s.find('final JSONArray historyData=a;')
if start >= 0:
    end=s.find('});', start)
    if end >= 0:
        block=s[start:end]
        block=block.replace('a.length()', 'historyData.length()').replace('a.getJSONObject(', 'historyData.getJSONObject(')
