...
public class MainActivity extends Activity {
    ...
    private void showAdminHome(){buildShell("ניהול",this::showHome,false);String[] labels={"מוצרים וקטגוריות","מלאי","ספקים","רכישה חדשה","היסטוריית רכישות","דוחות"};Runnable[] actions={this::showProductsAdmin,this::showStockAdmin,this::showSuppliersAdmin,this::showNewPurchase,this::showPurchaseHistory,this::showReports};for(int i=0;i<labels.length;i++){Button b=button(labels[i],Color.WHITE,blue);b.setOnClickListener(v->actions[i].run());LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(64));p.setMargins(0,0,0,dp(12));content.addView(b,p);}Button logout=button("יציאה מניהול",Color.rgb(220,38,38),Color.WHITE);logout.setOnClickListener(v->{adminToken="";showHome();});content.addView(logout,new LinearLayout.LayoutParams(-1,dp(60)));}
