...
helpers=r'''    private void setPaymentInputMode(boolean reader){
        ...
        if(paymentFrameWrap!=null){android.view.ViewGroup.LayoutParams raw=paymentFrameWrap.getLayoutParams();if(raw instanceof LinearLayout.LayoutParams){LinearLayout.LayoutParams lp=(LinearLayout.LayoutParams)raw;if(reader){lp.height=dp(1);lp.weight=0f;}else{lp.height=0;lp.weight=1f;}paymentFrameWrap.setLayoutParams(lp);}paymentFrameWrap.setAlpha(reader?0f:1f);paymentFrameWrap.setVisibility(View.VISIBLE);}
        if(paymentWebView!=null){paymentWebView.setVisibility(View.VISIBLE);paymentWebView.setAlpha(1f);if(reader){paymentWebView.requestFocus();main.postDelayed(()->sendPaymentWebKey(android.view.KeyEvent.KEYCODE_TAB),180);}}
    }
    ...
    private void finishCardReaderSwipe(){
        String raw=paymentCardReaderBuffer.toString();paymentCardReaderBuffer.setLength(0);java.util.regex.Matcher m=java.util.regex.Pattern.compile("(\\d{12,19})[=D](\\d{4})").matcher(raw);
        if(!m.find()){if(paymentReaderStatus!=null)paymentReaderStatus.setText("לא הצלחתי לקרוא את הכרטיס. נסה שוב");return;}
        String pan=m.group(1),expiryYYMM=m.group(2);String expiry=expiryYYMM.substring(2,4)+expiryYYMM.substring(0,2);if(paymentReaderStatus!=null)paymentReaderStatus.setText("הכרטיס נקרא בהצלחה — מבצע תשלום...");paymentCardReaderMode=false;
        if(paymentWebView!=null){paymentWebView.requestFocus();main.postDelayed(()->{sendPaymentWebDigits(pan);sendPaymentWebKey(android.view.KeyEvent.KEYCODE_TAB);sendPaymentWebDigits(expiry);main.postDelayed(()->chargeCard(),500);},80);}
    }
...
if 'paymentManualToggleButton=button(' not in pay:
    selector='''paymentReaderStatus=text("אנא העבר כרטיס לתשלום",20,true);paymentReaderStatus.setGravity(Gravity.CENTER);paymentReaderStatus.setTextColor(Color.rgb(43,119,139));body.addView(paymentReaderStatus,new LinearLayout.LayoutParams(-1,dp(48)));\n        paymentManualToggleButton=button("להזנת פרטי כרטיס לחץ כאן",Color.rgb(237,241,247),Color.DKGRAY);paymentManualToggleButton.setOnClickListener(v->setPaymentInputMode(!paymentCardReaderMode));body.addView(paymentManualToggleButton,new LinearLayout.LayoutParams(-1,dp(44)));\n        '''
    if frame_marker not in pay: raise SystemExit('frame marker not found')
    pay=pay.replace(frame_marker,selector+frame_marker,1)
# Keep a reference to the WRAPPER. The wrapper owns the weighted height, not the WebView itself.
if 'paymentFrameWrap=frameWrap;' not in pay:
    pay=pay.replace('LinearLayout frameWrap=new LinearLayout(this);','LinearLayout frameWrap=new LinearLayout(this);paymentFrameWrap=frameWrap;',1)
...
print('Manual payment now restores the Nedarim fields and the app numeric keypad; reader mode keeps both hidden')
