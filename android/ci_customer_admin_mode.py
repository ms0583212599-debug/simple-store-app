...
if 'מצב ניהול פעיל — יציאה' not in s:
...
# The storefront styling scripts run before this injector, so accept both the base and the
# compact/tiny-tile button variants instead of depending on one exact old string.
base_decl='        Button add=button(p.stock>0?"הוסף לסל":"לא זמין",blue,Color.WHITE);add.setEnabled(p.stock>0);'
tiny_decl='        Button add=button(p.stock>0?"+  הוסף לסל":"לא זמין",p.stock>0?green:Color.LTGRAY,p.stock>0?Color.WHITE:Color.DKGRAY);add.setEnabled(p.stock>0);add.setTextSize(12);add.setPadding(0,0,0,0);'
base_new='        Button add=button(customerAdminMode?"✎ עריכת מוצר":(p.stock>0?"הוסף לסל":"לא זמין"),customerAdminMode?blue:blue,Color.WHITE);add.setEnabled(customerAdminMode||p.stock>0);'
tiny_new='        Button add=button(customerAdminMode?"✎ עריכת מוצר":(p.stock>0?"+  הוסף לסל":"לא זמין"),customerAdminMode?blue:(p.stock>0?green:Color.LTGRAY),customerAdminMode?Color.WHITE:(p.stock>0?Color.WHITE:Color.DKGRAY));add.setEnabled(customerAdminMode||p.stock>0);add.setTextSize(12);add.setPadding(0,0,0,0);'
listener='        add.setOnClickListener(v->{int now=cart.getOrDefault(p.id,0);if(now<p.stock){cart.put(p.id,now+1);updateCartButton();Toast.makeText(this,"נוסף לסל",Toast.LENGTH_SHORT).show();}});'
listener_new='        add.setOnClickListener(v->{if(customerAdminMode){productDialog(p);return;}int now=cart.getOrDefault(p.id,0);if(now<p.stock){cart.put(p.id,now+1);updateCartButton();Toast.makeText(this,"נוסף לסל",Toast.LENGTH_SHORT).show();}});\n        if(customerAdminMode)card.setOnClickListener(v->productDialog(p));'
if 'customerAdminMode?"✎ עריכת מוצר"' not in s:
    if tiny_decl in s:
        s=s.replace(tiny_decl,tiny_new,1)
    elif base_decl in s:
        s=s.replace(base_decl,base_new,1)
    else:
        raise SystemExit('product add button marker not found')
    if listener not in s: raise SystemExit('product add listener marker not found')
...
logout_old='logout.setOnClickListener(v->{adminToken="";adminUserId="";getSharedPreferences("simple_store_auth",MODE_PRIVATE).edit().clear().apply();showHome();});'
logout_new='logout.setOnClickListener(v->{adminToken="";adminUserId="";customerAdminMode=false;getSharedPreferences("simple_store_auth",MODE_PRIVATE).edit().clear().apply();showHome();});'
if logout_old in s:
