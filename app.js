...
(function(){
  const V='20260901-customer-admin-mode1';
  ...
  async function boot(){
    ...
    if(document.readyState==='complete'){
      try{await authState();await load();totals()}catch(e){console.error('core init failed',e)}
    }
  ...
  window.loadAdminExtras=function(){
    ...
    adminExtrasPromise=Promise.all([script('sales-report.js'),script('purchase-import.js'),script('purchase-edit-enhancements.js'),script('contextual-excel.js'),script('finance-management.js'),script('admin-password.js'),script('inventory-edit.js'),script('customer-admin-mode.js')]).then(async()=>{
