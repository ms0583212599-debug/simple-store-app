...
async function renderEditor(){let host=document.getElementById('inventoryHistory');if(!host)return;try{cache=await req('/rest/v1/inventory_movements?select=*&order=created_at.desc&limit=500')||[]}catch(e){console.warn(e);return}if(!cache.length)return;
  ...
  // Fallback when the core history is rendered as cards rather than a table.
